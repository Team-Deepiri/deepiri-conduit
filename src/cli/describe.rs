...
pub async fn run(args: DescribeArgs, cli: &GlobalOpts) -> Result<()> {
    ...
    if let Some(c) = cfg {
        ...
        println!(
            "  {} {}",
            "Working Dir:".bold(),
            c.working_dir.as_deref().unwrap_or("—")
        );
    }
    println!();
        ...
        if let Some(env) = &c.env {
            ...
            for line in env.iter().take(15) {
                if let Some((k, v)) = line.split_once('=') {
                    let masked = if k.to_lowercase().contains("pass")
    ...
    if let Some(c) = cfg {
        if let Some(labels) = &c.labels {
            let conduit_labels: Vec<_> = labels
                .iter()
                .filter(|(k, _)| k.starts_with("conduit.") || k.starts_with("traefik."))
                .collect();
